    ...
    defaults : {
        ...
        prototype : {
            ...
            init : function() {
                ...
                $(this.currentForm).validateDelegate(
                        ":text, [type='password'], [type='file'], select, textarea, " + "[type='number'], [type='search'] ,[type='tel'], [type='url'], " + "[type='email'], [type='datetime'], [type='date'], [type='month'], "
                                + "[type='week'], [type='time'], [type='datetime-local'], " + "[type='range'], [type='color'] ", "focusin focusout keyup", delegate).validateDelegate(
                        "[type='radio'], [type='checkbox'], select, option", "click", delegate);
        ...
        methods : {
            ...
            date : function(value, element) {
                ...
                // return this.optional(element) ||
                // !/Invalid|NaN/.test(new Date(dateArray[0],
                // dateArray[1], dateArray[2]).toString());
            },
...
(function($) {
    ...
    $.validator.setDefaults({
        showErrors : function(errorMap, errorList) {
            ...
            function messageTarget ($element) {
                return $form.find(".form-alert").filter("[message-target='" + $element.attr("name") + "']");
            }
            $.each(this.validElements(), function (index, element) {
                var $element = $(element),
                    $messageTarget = messageTarget($element);
                $messageTarget.empty();
                $messageTarget.removeClass(["ver2", "on"]);
                // 같은 레벨에 여러개의 오류 메세지가 있다면 제일 처음 에러 메세지만 출력한다.
                $messageTarget.parent().children(".form-alert.on").first().show().end().not(":first").hide();
            });
            $.each(errorList, function (index, error) {
                var $element = $(error.element),
                $messageTarget = messageTarget($element);
                var errorMessage = $messageTarget.data("name") || "";
                errorMessage += error.message;
                $messageTarget.text(errorMessage).addClass(["ver2", "on"]);
                // 같은 레벨에 여러개의 오류 메세지가 있다면 제일 처음 에러 메세지만 출력한다.
                $messageTarget.parent().children(".form-alert.on").first().show().end().not(":first").hide();
            });
            if (this.checkForm()) {
                $form.find(".btn-type-text1").filter(":not(.zipcode)").removeClass("disabled");
            } else {
                $form.find(".btn-type-text1").filter(":not(.zipcode)").addClass("disabled");
            }
        }
    });
    ...
    $.extend($.validator.messages, {
        ...
        engAndNum : "은(는) 숫자와 영문을 포함해서 입력해 주십시오.",
        engAndNumAndSl : "은(는) 숫자와 영문, 특수문자를 포함해서 입력해 주십시오.",
        engOrNum : "은(는) 숫자,영문만 입력해 주십시오.",
        engOrNumOrSl : "은(는) 영문, 숫자 및 특수문자만 입력해 주십시오.",
        remote : "수정해 주시기 바랍니다.",
        emailId : " 아이디 형식을 올바로 입력해 주십시오.",
        emailDomain : " 도메인 형식을 올바로 입력해 주십시오.",
        email : "을(를) 올바로 입력해 주십시오.",
        maxbyte : $.validator.format("최대 {0}byte까지 입력이 가능합니다."),
        url : "을(를) 올바로 입력해 주십시오.",
        date : "날짜가 잘못 입력되었습니다.",
        dateISO : "ISO 형식에 맞는 날짜로 입력해 주십시오.",
        number : "은(는) 숫자만 입력해 주십시오.",
        digits : "숫자(digits)만 입력해 주십시오.",
        creditcard : "올바른 신용카드 번호를 입력해 주십시오.",
        equalTo : "값이 서로 다릅니다.",
        accept : "승낙해 주십시오.",
        maxlength : $.validator.format("은(는) {0}글자 이상은 입력할 수 없습니다."),
        minlength : $.validator.format("은(는) 적어도 {0}글자는 입력해야 합니다."),
        rangelength : $.validator.format("은(는) {0}글자 이상 {1}글자 이하로 입력해 주십시오."),
        range : $.validator.format("은(는) {0}에서 {1} 사이의 값을 입력해 주십시오."),
        max : $.validator.format("은(는) {0} 이하로 입력해 주십시오."),
        min : $.validator.format("은(는) {0} 이상으로 입력해 주십시오.")
    });
