                        ...
                        <div class="box-reward-inner">
                            ...
                            <div class="box-reward-content">                    
                                ...
                                <p class="text-normal-type4">
                                    <span class="text-point3">
                                                                                                                                ※ 예상수익은 조회기간 동안 발생한 가입건수를 기준으로 산정됩니다.<br>  
                                                                                                                                ※ 레디플래너가 모집한 회원의 가입절차는 ① 접수 → ② 가입대기 → ③ 가입순으로 진행됩니다.<br>
                                     &nbsp;&nbsp;① [접수건수]는 모집한 회원이 레디플래너의 고유링크로 버킷마켓 접속 후 상품 상담신청을 한 상태입니다.<br>
                                     &nbsp;&nbsp;② [가입대기건수]는 회원이 상품가입을 희망하여 상품 TM녹취계약을 완료한 상태입니다.<br>
                                     &nbsp;&nbsp;③ [가입건수]는 회원이 가입한 상품의 월 납입금을 1회 이상 납입완료한 상태입니다.<br>
                                     &nbsp;&nbsp;(실제 수익발생 기준이며, 수익은 1회 정상납입 및 유지 조건하에 지급됩니다.)<br>
                                                                                                                                ※ 실제 수익은 예상수익에서 해지(청약철회, 해약)회원에 대한 수익은 제외되고 지급됩니다.
                                    </span>
                                </p>
                            </div>
                            <!-- end -->
...
<form name="productPop" id="productPop">
    <input type="hidden" name="searchStartDate">
    <input type="hidden" name="searchEndDate">
    <input type="hidden" name="searchContractType">
    <input type="hidden" name="searchPrdMstNm">
</form>
...
<script id="productItemTemplate" type="text/x-jquery-tmpl">
    <tr class="row">
        ...
        <td class="list-detail" data-nm="{{= PRD_MST_NM }}" data-type="${Code.CONTRACT_STATUS_RECEIPT }">{{= StringUtil.setComma(TOTAL_RECEIPT) }}</td>
        <td class="list-detail" data-nm="{{= PRD_MST_NM }}" data-type="${Code.CONTRACT_STATUS_WAITING_TO_JOIN }">{{= StringUtil.setComma(TOTAL_WAITING_TO_JOIN) }}</td>
        <td class="list-detail" data-nm="{{= PRD_MST_NM }}" data-type="${Code.CONTRACT_STATUS_JOIN }">{{= StringUtil.setComma(TOTAL_JOIN) }}</td>
    </tr>
</script>
...
<script>
    $(function () {
        ...
        //상품별 상세조회
        $areaContent.on("click", ".list-detail", function(){
            $("#productPop").find("input[name='searchStartDate']").val($productSearchStartDate.val());
            $("#productPop").find("input[name='searchEndDate']").val($productSearchEndDate.val());
            $("#productPop").find("input[name='searchContractType']").val($(this).data("type"));
            $("#productPop").find("input[name='searchPrdMstNm']").val($(this).data("nm"));
            $("#productPop").postPop($.action.myplanner.performance.product.history(), "plannerStatusHistoryPopup", "width=1600,height=800,left=50,top=50");
        });        
    });
</script>
</body>
