...
<head>
    ...
    <%@include file="/WEB-INF/jsp/pc/include/head_none_meta.jsp" %>
...
enp('create', 'conversion', 'bucketmarket', { device: 'W', btnSelector:'.button-join-contract', convType: 'counsel', productName: '<c:out value="${info.PRD_MST_NO}"/>'});
</script>
<!-- Enliple Tracker End -->
<!-- TRADINGWORKS SCRIPT-->
                ...
                <div class="contain-product1">
                    ...
                    <c:if test="${not (info.PRD_MST_TYPE eq Code.PRODUCT_TYPE_IMMEDIATELY_ISSUE and info.PRD_MST_BND_YN eq 'Y') && info.PRD_MST_TYPE ne Code.PRODUCT_TYPE_DAEMYUG }">
                        <div class="box-detail-product2">
                            <dl>
                                ...
                                <dd class="detail2">(<c:if test="${info.PRD_MST_TYPE eq Code.PRODUCT_TYPE_MONTHLY_INSTALLMENT_PLAN }">할부이자 포함 </c:if>총 지원혜택 <fmt:formatNumber value="${info.REL_AMT }" type="number"/>원)</dd>
                            </dl>
                        </div>
                        <div class="box-detail-product3">
                            <dl>
                    ...
                    <div class="box-button">
                        <c:if test="${info.PRD_MST_DPL_YN eq  Product.DISPLAY_SHOW }">
                            <span class="btn-apply-join button-join-contract <c:if test="${info.PRD_MST_OPT_YN eq 'Y' }">disabled</c:if>"><button onclick="gtag_report_conversion();">지금 상담 신청하기 ➤</button></span>
                        </c:if>
                        ...
                        <span class="btn-share-detail"><button class="call-sns-layer">상품 공유하기</button></span>
                    </div>
                </div>
...
<%--                                </c:if>--%>
                            ...
                            <c:if test="${info.PRD_MST_DPL_YN eq  Product.DISPLAY_SHOW }">
                                <span class="btn-type1 button-join-contract disabled"><button onclick="gtag_report_conversion();">지금 상담 신청하기 ➤</button></span>
                            </c:if>
                            <c:if test="${info.PRD_MST_DPL_YN eq  Product.DISPLAY_SOLDOUT }">
                ...
                <c:if test="${info.PRD_MST_REV_YN eq 'Y' }">
                    <div class="contain-product6 product-review-box">
                        <div class="box-inner">
                            ...
                            <span class="btn-type-text4"><button>상품후기 전체보기</button></span>
...
<%--                            <span class="btn-type-text4"><button onclick="$.requestPage($.action.board.comment.list());">모든 상품의 BEST 후기 보기</button></span>--%>
    ...
    <script id="commentBoardListTemplate" type="text/x-jquery-tmpl">
        {{if StringUtil.getInt(totalCount) == 0}}
            <p class="text-empty-data">등록된 상품후기가 없습니다.</p>
        {{else}}
            {{each(i, row) list}}
                <div class="box-latter-view">
    ...
    <script id="productInfoTemplate" type="text/x-jquery-tmpl">
        ...
        <div class="contain-body">
            <div class="list-type1">
                <ul>
                    <li>선택하신 상품은 제품 사전결제가 필요한 상품입니다.<br >지원혜택 제품가격 <span class="text-point2"><fmt:formatNumber value="${info.PRD_MST_ISTM_SPT_PAY}" type="number"/>원</span>에 대한 할부결제 진행</li>
                    <li>라이프서비스 부금에서 <span class="text-point2"><fmt:formatNumber value="420000" type="number"/>원</span> 할인 <br><small>(할부이자 포함 지원)</small></li>
                    <li>만기 시엔 결제하신 금액을 포함하여 납입하신 <span class="text-point2"><fmt:formatNumber value="${info.PROD_AMT}" type="number"/>원</span> 전액 환급</li>
    ...
    <script id="productPreviewTemplate" type="text/x-jquery-tmpl">
        ...
        <span class="btn-close"><button>확인</button></span>
    </script>
    <script>
        $(function () {
            ...
            <%-- 상품 리뷰 전체보기 호출 --%>
            $areaBody.on("click", ".btn-type-text4", function () {
                var $this = $(this);
                var latterList = $(".box-latter-body.list-area");
                var pageing = $(".paginate");
                if( latterList.hasClass("on") ) {
                    latterList.removeClass("on");
                    pageing.removeClass("on")
                    $this.find("button").text("상품후기 전체보기");
                } else {
                    latterList.addClass("on");
                    pageing.addClass("on")
                    $this.find("button").text("상품후기 닫기");
                }
            });
