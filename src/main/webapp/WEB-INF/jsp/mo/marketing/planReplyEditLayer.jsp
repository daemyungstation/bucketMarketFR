...
<div class="area-content-layer area-pad">
    <div class="area-inner">
        <form name="replyForm" id="replyForm" onsubmit="return false">
            ...
            <input type="hidden" name="PLN_RPL_IDX" value="<c:out value="${commandMap.PLN_RPL_IDX }"/>"/>
            <div class="contain-comment">
                <span class="form-textarea">
                    <textarea name="PLN_RPL_CTS" id="PLN_RPL_CTS" placeholder="내용을 입력해 주세요" onkeyup="StringUtil.getStrLength(this, '.byte', 600);" maxlength="600"><c:out value="${info.PLN_RPL_CTS }"/></textarea>
                    <i><span class="byte"><c:out value="${fn:length(info.PLN_RPL_CTS) }"/></span>/600</i>
                    <label class="form-alert" message-target="PLN_RPL_CTS" data-name="내용"></label>
                </span>
                <div class="box-form">
                    <span>
                        <input type="text" name="PLN_RPL_NM" id="PLN_RPL_NM" value="<c:out value="${info.PLN_RPL_NM }"/>" placeholder="이름" maxlength="10"/>
                        <label class="form-alert" message-target="PLN_RPL_NM" data-name="이름"></label>
                    </span>
                    <span>
                        <input type="password" name="PLN_RPL_PW" id="PLN_RPL_PW" value="" placeholder="비밀번호"/>
