...
<jsp:include page="/WEB-INF/jsp/mo/include/header.jsp" flush="false">
    ...
    <jsp:param name="headerTitle" value="납입/환급금 조회"/>
</jsp:include>
<!-- //헤더 -->
<!-- 카테고리 -->
        ...
        <c:choose>
            ...
            <c:when test="${fn:length(list) gt 0 }">
                ...
                <div class="area-mypage contain-payment">
                    ...
                    <p class="text-information-type2">* 가입한 상품의 상세 납입현황을 조회합니다.<br />청약철회/행사/해약 상품 내역은 표시 되지 않습니다.</p>
                    <p class="text-information-type2">* 자세한 납입정보는 [납입증서] 를 참고하시기 바랍니다.</p>
                    ...
                    <c:forEach var="item" items="${list }">
                        ...
                        <div class="contain-history-subscripton1" data-idx="<c:out value="${item.ORD_MST_IDX }"/>">
                            ...
                            <div class="box-button">
                                <span class="btn-type-text7"><button class="certificate">납입증서</button></span>
                                <span class="btn-type-text7"><button class="detail">납입상세</button></span>
                                <span class="btn-type-text7"><button class="refund">환급금 조회</button></span>
                            </div>
                        </div>
                    ...
                    <p class="text-information-type2">* 납입금액은 상조부금, 결합상품 금액, 추가부담금 등을 종합하여 표기 됩니다.</p>
                    <p class="text-information-type2">* 납입내역은 [납입상세]를 통해 기간별 납입내역을 조회하실 수 있습니다.</p>
...
<script>
    $(function () {
        ...
        // 납입상세
        $areaContent.on("click", "button.detail", function() {
            var params = { ORD_MST_IDX : $(this).closest("div.contain-history-subscripton1").attr("data-idx") }
            $(".layer-popup1").requestLayer($.action.mycontract.paymentrefund.detail.view.layer(), params);
        });
        // 환급금 조회
        $areaContent.on("click", "button.refund", function() {
            var params = { ORD_MST_IDX : $(this).closest("div.contain-history-subscripton1").attr("data-idx") }
            $(".layer-popup1").requestLayer($.action.mycontract.paymentrefund.refund.view.layer(), params);
        });
    });
</script>
