    ...
    <div class="area-content">
        <div class="area-inner">
            ...
            <div class="contain-product1">
                ...
                <c:if test="${not (info.PRD_MST_TYPE eq Code.PRODUCT_TYPE_IMMEDIATELY_ISSUE and info.PRD_MST_BND_YN eq 'Y') }">
                    <div class="box-detail-product2">
                        <dl>
                            ...
                            <dd class="detail1"><fmt:formatNumber value="${info.MON_REL_AMT }" type="number"/>원</dd>
                            <dd class="detail2"><fmt:formatNumber value="${info.REL_CNT }" type="number"/>회 지원</dd>
                            <dd class="detail3">(총 지원혜택 <fmt:formatNumber value="${info.REL_AMT }" type="number"/>원)</dd>
                        </dl>
                    </div>
                ...
                <c:if test="${not empty typeInfo.PRD_TPL_MB_DTL_DSC}">
                    <div class="box-detail-product5">
                        <div class="box-media">
                            <c:out value="${typeInfo.PRD_TPL_MB_DTL_DSC }" escapeXml="false"/>
                        </div>
                    </div>
                </c:if>
            ...
            <div class="contain-product2">
                <div class="contain-inner">
                    ...
                    <div class="box-tab-body tab1 on">
                        <c:if test="${not empty info.PRD_MST_MB_YTB }">
                            <div class="box-media">
                                <c:out value="${info.PRD_MST_MB_YTB }" escapeXml="false"/>
                            </div>
                        </c:if>
                        <div class="box-inner">
            ...
            <c:if test="${info.PRD_MST_QNA_YN eq 'Y' }">
                ...
                <div class="contain-product5">
                    <div class="box-inner">
                        ...
                        <c:if test="${productQnACount gt 3 }">
            ...
            <c:if test="${info.PRD_MST_REV_YN eq 'Y' }">
                ...
                <div class="contain-product6">
                    <div class="box-inner">
                        ...
                        <c:if test="${empty commentBoardList }">
                            <p class="text-empty-data">등록된 가입후기가 없습니다.</p><%-- 데이터가 없을때 --%>
                            <span class="btn-type-text4"><button onclick="$.requestPage($.action.board.comment.list());">모든 상품의 BEST 후기 보기</button></span>
                        </c:if>
                        <c:if test="${not empty commentBoardList }">
                            ...
                            <c:if test="${commentBoardCount gt 3 }">
                                <span class="btn-type-text4"><button class="call-list-layer" data-type="comment-board-list">이 상품에 가입후기 전체보기</button></span>
                            </c:if>
                        </c:if>
                    </div>
                </div>
                        ...
                        <c:when test="${(info.PRD_MST_TYPE eq Code.PRODUCT_TYPE_MONTHLY_SUPPORT or info.PRD_MST_TYPE eq Code.PRODUCT_TYPE_IMMEDIATELY_ISSUE) and info.PRD_MST_JOIN_LMT gt 1 }">
                            <div class="box-option2<c:if test="${info.PRD_MST_JOIN_LMT gt 1 }"> on</c:if>">
                                <div class="box-inner">
                                    <p class="text-normal-type1">가입 구좌를 선택해 주세요.</p>
                                    <div class="box-increase-type1">
                                        <input type="number" name="ORD_MST_CNT" id="ORD_MST_CNT" class="number input-order-count" value="1" maxlength="2" readonly="readonly"/>
                                        <span class="btn-increase"><button onclick="return false;">-</button></span>
                                        <span class="btn-decrease"><button onclick="return false;">+</button></span>
                                    </div>
            ...
            <div class="fixed-box-button">
                <c:if test="${info.PRD_MST_DPL_YN eq  Product.DISPLAY_SHOW }">
                    <span class="btn-type-text1 <c:if test="${info.PRD_MST_OPT_YN eq 'Y' }">disabled</c:if>"><button class="product-order">간편가입신청</button></span>
                </c:if>
    ...
    <script id="productInfoTemplate" type="text/x-jquery-tmpl">
        ...
        <div class="contain-body">
            <div class="list-type1">
                <ul>
                    <li>선택하신 상품은 사전결제가 필요한 상품입니다. <br >지원혜택 제품가격 <span class="text-point1"><fmt:formatNumber value="${info.PRD_MST_ISTM_SPT_PAY}" type="number"/>원</span>에 대한 결제 진행</li>
                    <li>상조부금에서 <span class="text-point1"><fmt:formatNumber value="${info.PRD_MST_ISTM_SPT_PAY}" type="number"/>원</span> 할인</li>
                    <li>만기 시엔 결제하신 금액을 포함하여 납입 하신 <span class="text-point1"><fmt:formatNumber value="${info.PROD_AMT}" type="number"/>원</span> 전액 환급 확인 시 상품가입 페이지로 이동합니다.</li>
                </ul>
            </div>
