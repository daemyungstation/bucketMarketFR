...
public class MyPlannerCampaignController extends ControllerSupport {
    ...
    /**
     * <pre>
     * 1. MethodName : campaignCreateIntroLayer
     * 2. ClassName  : MyPlannerCampaignController.java
     * 3. Comment    : 캠페인 생성 (layer)
     * 4. 작성자       : inus
     * 5. 작성일       : 2021. 11. 26.
     ...
     */
    @AccessPlannerRole(ROLE_PLANNER.planner)
    @RequestMapping(value = "${myplanner.campaign.create.intro.view.layer}")
    public ModelAndView campaignCreateIntroLayer(HttpServletRequest request, @CommandMap Map<String, Object> commandMap) throws Exception {
        ModelAndView mv = new ModelAndView();
        // 상품 정보
        mv.addObject("info", this.myPlannerCampaignService.selectProductIntroInfo(commandMap));
        mv.addObject("sellerUrlList", this.myPlannerCampaignService.selectCampaignSellerShortenUrlList(commandMap));
        mv.addObject("commandMap", commandMap);
        setViewName(mv, "myplanner.campaign.create.intro.view.layer");
        return mv;
    }
    ...
    /**
     * <pre>
     * 1. MethodName : campaignCreateSellerLayer
     * 2. ClassName  : MyPlannerCampaignController.java
     * 3. Comment    : 판매원용 캠페인 생성 (layer)
     * 4. 작성자       : inus
     * 5. 작성일       : 2022. 03. 04.
     * </pre>
     *
     ...
     */
    @AccessPlannerRole(ROLE_PLANNER.planner)
    @RequestMapping(value = "${myplanner.campaign.create.seller.view.layer}")
    public ModelAndView campaignCreateSellerLayer(HttpServletRequest request, @CommandMap Map<String, Object> commandMap) throws Exception {
        ModelAndView mv = new ModelAndView();
        // 캠페인 URL 목록
        //mv.addObject("list", this.myPlannerCampaignService.selectCampaignSellerShortenUrlList(commandMap));

        mv.addObject("commandMap", commandMap);
        setViewName(mv, "myplanner.campaign.create.seller.view.layer");
        return mv;
    }
     ...
     */
    @AccessPlannerRole(ROLE_PLANNER.planner)
    @RequestMapping(value = "${myplanner.campaign.create.seller.ajax}", method = RequestMethod.POST)
    public JSON campaignCreateSellerAjax(HttpServletRequest request, @CommandMap Map<String, Object> commandMap) throws Exception {
        JSON json = new JSON();
        json.addObject("list", this.myPlannerCampaignService.selectCampaignSellerShortenUrlList(commandMap));
