...
//        });
    ...
    //팝업 위치
    if($('.layer-popup1, .layer-popup2, .layer-popup4, .layer-popup5').length > 0){
        posPopup()
    }else{}
...
//팝업 위치
function posPopup(){
    $('.layer-popup1, .layer-popup2, .layer-popup4, .layer-popup5').each(function(){
...
function posPopup2(layerClassname, layerInnerClassname){
    $(layerClassname).each(function(){
        var winHeight = $(window).height() / 2
        var popPopY = winHeight - 30 - $(this).find(layerInnerClassname).innerHeight() / 2
